...
public class CacheAsSessionDemo extends Controller {
	...
	public static Result index() {
		return ok(cacheassessiondemo.render(pageTitle, TimeHelper.getCurrentTime(), defaultForm,
	...
	public static Result setValueInCacheWithUUID() {
		...
		if (form.hasErrors()) {
			return badRequest(cacheassessiondemo.render(pageTitle, TimeHelper.getCurrentTime(), form,
		...
		if (cachedValue == null) {
			return ok(cacheassessiondemo.render(pageTitle, TimeHelper.getCurrentTime(), defaultForm, "The value "
					+ value + " was not set successfully in the cache"));
		}

		return ok(cacheassessiondemo.render(pageTitle, TimeHelper.getCurrentTime(), defaultForm, "The value " + value
	...
	public static Result getValueInCacheWithUUID() {
		...
		if (cachedValue == null) {
			return ok(cacheassessiondemo.render(pageTitle, TimeHelper.getCurrentTime(), defaultForm, "There is no value in the cache for the UUID " + uuid));
		}

		return ok(cacheassessiondemo.render(pageTitle, TimeHelper.getCurrentTime(), defaultForm, "The value " + cachedValue
